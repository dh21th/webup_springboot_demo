...
public class ElasticsearchTest2 {
    ...
    public void getClient() throws Exception{
        //设置集群名称
        Settings settings = Settings.builder().put("cluster.name", "zsy").build();// 集群名
        client = new PreBuiltTransportClient(settings);//6.3.2这里TransportAddress代替InetSocketTransportAddress
        client.addTransportAddress(new TransportAddress(new InetSocketAddress(InetAddresses.forString("10.6.101.50"), 9300)));
...
//                + "\n文档ID:" + response.getId() + "\n当前实例状态:" + response.status());
    ...
    public void autoBulkProcessor() throws Exception {
        ...
        for(int i=0;i<80080;i++){
            ...
            bulkProcessor.add(new IndexRequest("article", "content",""+i).source(JSON.parseObject(json)));//添加文档，以便自动提交
        }
...
//                        "getTookInMillis : " + response.getIngestTookInMillis() + "\n" +
